...
geometry = [['H', (i*spacing, 0.0, 0.0)] for i in range(4)]
charge=0
basis='sto3g'
multiplicity=1
molecule = MolecularData(
...
max_bond_list = np.round(np.linspace(2, max_bond, 2)).astype(int).tolist()
gap_dict = {}
for mb in max_bond_list:
    mps = MatrixProductState.from_dense(gs)
    n_qubits = int(np.log2(len(gs)))
    
    mps.compress(max_bond=mb, cutoff=1e-10, method='svd')
    mps.show()
    state = mps.to_dense().flatten()
    ...
    eigenvalues_conj, eigenvectors_conj = np.linalg.eigh(H_conj)
    diff_array = []
    for s in s_array:
...
plt.title('Min Gap and Overlap vs max layer of entangler for H4 chain')
plt.legend()
